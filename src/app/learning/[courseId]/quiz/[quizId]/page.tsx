...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    const checkQuizUnlock = async () => {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://cybertest-frontend-production.up.railway.app'}/api/progress/quiz/${quizId}/unlock`, {
          headers: {
    ...
    const loadPreviousAttempt = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://cybertest-frontend-production.up.railway.app'}/api/progress/quiz-attempts`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const fetchQuiz = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://cybertest-frontend-production.up.railway.app'}/api/courses/${courseId}`);
        const data = await response.json();
